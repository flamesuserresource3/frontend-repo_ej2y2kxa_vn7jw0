    ...
    <section id="menu" className="relative py-20 bg-white">
      <div className="mx-auto max-w-6xl px-4">
        <div className="max-w-2xl">
          <h2 className="text-3xl sm:text-4xl font-bold text-pink-900">Menu</h2>
          <p className="mt-3 text-pink-700/80">Handcrafted in small batches. Rotating seasonally.</p>
        </div>
        <div className="mt-10 grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6">
          {MENU.map((f) => (
            <div key={f.id} className="group rounded-2xl border border-pink-100 bg-white p-2 shadow-sm hover:shadow-md transition">
              ...
              <div className="p-4">
                <div className="flex items-start justify-between">
                  <h3 className="font-semibold text-pink-900">{f.name}</h3>
                  <span className="text-xs rounded-full bg-pink-100 px-2 py-1 text-pink-700">${f.price.toFixed(2)}</span>
                </div>
                <p className="mt-1 text-sm text-pink-800/80">{f.desc}</p>
                <button
                  onClick={() => addItem({ id: f.id, name: f.name, price: f.price, image: f.image })}
                  className="mt-4 inline-flex items-center justify-center w-full rounded-md bg-pink-600 text-white text-sm py-2 hover:bg-pink-700 transition"
                >
                  Add to Cart
                </button>
              </div>
